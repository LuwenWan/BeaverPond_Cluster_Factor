...
library(ggrepel) # provides functions to automatically adjust label positions to prevent overlap
library(readr)
library(ggspatial)
library(mgcv) 
...
# 3.0 start here - save to save time 
```{r}
# save(pond_polygon_eco_cluster, file = paste0(outdir,"/pond_polygon_eco_cluster.RData"))
load(paste0(outdir,"/pond_polygon_eco_cluster.RData"))
pond_polygon_eco_cluster_df <- st_drop_geometry(pond_polygon_eco_cluster)
```


# 3. plot ponds in ecoregions 
## 3.0 pond and ecoregion for table S1
```{r}
...
## 3.3 level III ecoregion (US or NA is the same)
```{r}
# Define ecoregion colors with descriptive names including codes
ecoregion_colors <- c(
...
## 3.4. save plot - ecoregion 

```{r}
fname <- paste0(outfig,'/ecoregion_level_III.png'); fname
